...
#[test]
fn chord_trie_commands_with_prefix_match() {
    let t = closure_input::ChordTrie::build(&[("a b", "x"), ("a c", "y"), ("d", "z")]);
    let mut v = t.commands_with_prefix("a");
    v.sort();
    assert_eq!(v, vec!["x".to_owned(), "y".to_owned()]);
}

#[test]
fn chord_trie_commands_with_prefix_empty_when_no_match() {
    let t = closure_input::ChordTrie::build(&[("a", "x")]);
    assert!(t.commands_with_prefix("z").is_empty());
}

#[test]
fn dispatcher_commands_with_prefix_match() {
    let mut reg = Registry::new();
    reg.register(Box::new(RenameHeadline::new_placeholder()));
    let disp = Dispatcher::from_registry(&reg, InputMode::Doom);
    let v = disp.commands_with_prefix("C-c");
    assert_eq!(v, vec!["rename-headline".to_owned()]);
}

#[test]
fn dispatcher_commands_with_prefix_empty_when_no_match() {
    let mut reg = Registry::new();
    reg.register(Box::new(RenameHeadline::new_placeholder()));
    let disp = Dispatcher::from_registry(&reg, InputMode::Doom);
    assert!(disp.commands_with_prefix("Z-z").is_empty());
}

#[test]
fn chord_trie_chords_with_prefix_count_match() {
    let t = closure_input::ChordTrie::build(&[("a b", "x"), ("a c", "y"), ("d", "z")]);
    assert_eq!(t.chords_with_prefix_count("a"), 2);
    assert_eq!(t.chords_with_prefix_count("z"), 0);
}

#[test]
fn chord_trie_commands_with_prefix_count_match() {
    let t = closure_input::ChordTrie::build(&[("a b", "x"), ("a c", "y"), ("d", "z")]);
    assert_eq!(t.commands_with_prefix_count("a"), 2);
}

#[test]
fn dispatcher_chords_with_prefix_count_match() {
    let mut reg = Registry::new();
    reg.register(Box::new(RenameHeadline::new_placeholder()));
    let disp = Dispatcher::from_registry(&reg, InputMode::Doom);
    assert_eq!(disp.chords_with_prefix_count("C-c"), 1);
}

#[test]
fn dispatcher_commands_with_prefix_count_match() {
    let mut reg = Registry::new();
    reg.register(Box::new(RenameHeadline::new_placeholder()));
    let disp = Dispatcher::from_registry(&reg, InputMode::Doom);
    assert_eq!(disp.commands_with_prefix_count("C-c"), 1);
}
...
#[test]
fn chord_trie_has_node_at_depth_true_when_match() {
    let t = closure_input::ChordTrie::build(&[("a b", "x")]);
    assert!(t.has_node_at_depth(0));
    assert!(t.has_node_at_depth(1));
    assert!(t.has_node_at_depth(2));
    assert!(!t.has_node_at_depth(5));
}

#[test]
fn chord_trie_has_node_at_depth_false_when_empty() {
    let t = closure_input::ChordTrie::build(&[]);
    assert!(t.has_node_at_depth(0));
    assert!(!t.has_node_at_depth(1));
}
