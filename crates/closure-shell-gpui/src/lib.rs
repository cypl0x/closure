...
impl Shell {
    ...
    pub const fn new(vault: Vault) -> Self {
        Self {
            vault,
            selection: Selection { file: None, headline: 0 },
        }
    }
...
mod tests {
    ...
    fn test_vault() -> (TempDir, Vault) {
        ...
        fs::write(dir.path().join("notes.org"), "* TODO Test gpui\n:PROPERTIES:\n:ID: 01HQXGPUI0000000000000000\n:END:\n").expect("write");
    ...
    fn gpui_uses_registry_for_commands() {
        // I4/I8: mutations only through registry surface (even if direct vault for shell model).
        let reg = closure_core::default_registry();
        assert!(reg.get("rename-headline").is_some(), "gpui must align with registry");
    }
}
...
pub fn run(vault_path: &std::path::Path) -> Result<(), String> {
    ...
    println!("gpui polished variant ready — high-perf desktop shell (see GpuiPolishedView render for the UI)");
    Ok(())
}
    ...
    fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let items: Vec<_> = if self.query.is_empty() {
            shell
                ...
                .flat_map(|(p, d)| {
                    d.all_headlines()
                        .map(move |h| (p.display().to_string(), h.title().to_string(), h.level(), h.todo().map(|s| s.to_string())))
                })
                .collect()
        } else {
            shell
