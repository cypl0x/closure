...
impl OrgDoc {
    ...
    /// Most frequently appearing title (lowest name wins ties).
    #[must_use]
    ...
    /// Least frequently appearing title (lowest name wins ties).
    #[must_use]
    ...
    /// Returns every headline whose body contains `needle`.
    #[must_use]
    pub fn headlines_with_body_substring<'a>(&'a self, needle: &str) -> Vec<&'a Headline> {
        self.filter_headlines(|h| h.body_source().contains(needle))
    }

    /// Returns every headline (title or body) containing `needle`.
    #[must_use]
    pub fn full_text<'a>(&'a self, needle: &str) -> Vec<&'a Headline> {
    ...
    /// Returns the last headline with priority `letter` (DFS order).
    #[must_use]
    ...
    /// Median property-key length in characters (`None` when no properties).
    #[must_use]
    ...
    /// Histogram of property-key lengths (chars) to occurrence count.
    #[must_use]
    ...
    /// Most common property-key length (lowest wins ties).
    #[must_use]
    ...
    /// Distinct property keys across every headline in the document, sorted.
    #[must_use]
    ...
    /// Count of distinct property keys across the document.
    #[must_use]
    ...
    /// Percentage of source bytes that are headline headers
    /// (`total header bytes * 100 / source bytes`, `0` when source empty).
    #[must_use]
    pub fn header_byte_pct(&self) -> usize {
        (self.total_header_byte_len() * 100)
            .checked_div(self.source_byte_len())
            .unwrap_or(0)
    }
    ...
    pub fn body_line_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for h in self.iter_headlines() {
            *m.entry(h.body_line_count()).or_insert(0) += 1;
        }
        m
    }
    ...
    pub fn body_char_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for h in self.iter_headlines() {
            *m.entry(h.body_char_count()).or_insert(0) += 1;
        }
        m
    }
    ...
    /// Subtree link count for `id`-tagged headline.
    #[must_use]
    pub fn subtree_link_count_of(&self, id: &str) -> Option<usize> {
    ...
    /// Header line for `id`-tagged headline.
    #[must_use]
    pub fn header_of(&self, id: &str) -> Option<&str> {
        self.headline_by_id(id).map(Headline::header)
    }

    /// Display title for `id`-tagged headline.
    #[must_use]
    pub fn display_title_of(&self, id: &str) -> Option<String> {
    ...
    /// Returns the most-incoming-referenced id and its incoming count.
    #[must_use]
    ...
    /// Edges where target resolves inside the doc.
    #[must_use]
    ...
    /// Returns the most common priority letter and its count.
    #[must_use]
    pub fn modal_priority(&self) -> Option<(char, usize)> {
    ...
    /// Returns max subtree link count across all headlines (`0` when empty).
    #[must_use]
    pub fn max_subtree_link_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_link_count)
            .max()
            .unwrap_or(0)
    }

    /// Returns min subtree link count across all headlines (`0` when empty).
    #[must_use]
    pub fn min_subtree_link_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_link_count)
            .min()
    ...
    /// Returns total subtree link count summed across all headlines.
    #[must_use]
    ...
    /// Integer mean subtree link count across all headlines (`0` when empty).
    #[must_use]
    ...
    /// Median subtree link count across all headlines (`None` when empty).
    #[must_use]
    ...
    /// Histogram of per-headline subtree link counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree link count (lowest wins ties).
    #[must_use]
    ...
    /// Median subtree distinct-tag count across all headlines (`None` when empty).
    #[must_use]
    ...
    /// Histogram of per-headline subtree distinct-tag counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree distinct-tag count (lowest wins ties).
    #[must_use]
    ...
    /// Histogram of per-headline subtree property counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree property count (lowest wins ties).
    #[must_use]
    ...
    /// Histogram of per-headline subtree timestamp counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree timestamp count (lowest wins ties).
    #[must_use]
    ...
    /// Returns max subtree distinct-priority count across all headlines (`0` when empty).
    #[must_use]
    pub fn max_subtree_priority_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_priority_count)
            .max()
            .unwrap_or(0)
    }

    /// Returns min subtree distinct-priority count across all headlines (`0` when empty).
    #[must_use]
    pub fn min_subtree_priority_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_priority_count)
            .min()
    ...
    /// Returns total subtree distinct-priority count summed across all headlines.
    #[must_use]
    ...
    /// Integer mean subtree distinct-priority count across all headlines (`0` when empty).
    #[must_use]
    ...
    /// Histogram of per-headline subtree distinct-priority counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree distinct-priority count (lowest wins ties).
    #[must_use]
    ...
    /// Returns max subtree distinct-level count across all headlines (`0` when empty).
    #[must_use]
    pub fn max_subtree_level_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_level_count)
            .max()
            .unwrap_or(0)
    }

    /// Returns min subtree distinct-level count across all headlines (`0` when empty).
    #[must_use]
    pub fn min_subtree_level_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_level_count)
            .min()
    ...
    /// Returns total subtree distinct-level count summed across all headlines.
    #[must_use]
    ...
    /// Integer mean subtree distinct-level count across all headlines (`0` when empty).
    #[must_use]
    ...
    /// Median subtree distinct-level count across all headlines (`None` when empty).
    #[must_use]
    ...
    /// Histogram of per-headline subtree distinct-level counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree distinct-level count (lowest wins ties).
    #[must_use]
    ...
    /// Total body word count across roots only (no recursion).
    #[must_use]
    pub fn total_root_body_word_count(&self) -> usize {
        self.roots.iter().map(Headline::body_word_count).sum()
    }

    /// Maximum root body word count.
    #[must_use]
    pub fn max_root_body_word_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::body_word_count).max()
    }

    /// Minimum root body word count.
    #[must_use]
    pub fn min_root_body_word_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::body_word_count).min()
    }

    /// Integer mean root body word count (`0` when no roots).
    #[must_use]
    pub fn mean_root_body_word_count(&self) -> usize {
        let n = self.roots.len();
        self.total_root_body_word_count()
            .checked_div(n)
            .unwrap_or(0)
    }

    /// Median root body word count (`None` when no roots).
    #[must_use]
    pub fn median_root_body_word_count(&self) -> Option<usize> {
        let mut v: Vec<usize> = self.roots.iter().map(Headline::body_word_count).collect();
        if v.is_empty() {
            return None;
        }
        v.sort_unstable();
    ...
    /// Histogram of root body word counts to occurrence count.
    #[must_use]
    pub fn root_body_word_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for r in &self.roots {
            *m.entry(r.body_word_count()).or_insert(0) += 1;
        }
        m
    }
    ...
    /// Returns min tag count across headlines (`0` when empty).
    #[must_use]
    pub fn min_tag_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::tag_count)
            .min()
    ...
    /// Median per-headline tag count (`None` when empty).
    #[must_use]
    ...
    #[must_use]
    pub fn headline_tag_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for h in self.iter_headlines() {
            *m.entry(h.tag_count()).or_insert(0) += 1;
        }
        m
    }
    ...
    pub fn mode_tag_count(&self) -> Option<usize> {
        let mut best: Option<(usize, usize)> = None;
        for (tc, c) in self.headline_tag_count_counts() {
            if best.is_none_or(|(_, bc)| c > bc) {
                best = Some((tc, c));
            }
        }
    ...
    /// Total tag count across roots only (no recursion).
    #[must_use]
    pub fn total_root_tag_count(&self) -> usize {
        self.roots.iter().map(Headline::tag_count).sum()
    }

    /// Maximum root tag count.
    #[must_use]
    pub fn max_root_tag_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::tag_count).max()
    }

    /// Minimum root tag count.
    #[must_use]
    pub fn min_root_tag_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::tag_count).min()
    }

    /// Integer mean root tag count (`0` when no roots).
    #[must_use]
    pub fn mean_root_tag_count(&self) -> usize {
        let n = self.roots.len();
        self.total_root_tag_count().checked_div(n).unwrap_or(0)
    }

    /// Median root tag count (`None` when no roots).
    #[must_use]
    pub fn median_root_tag_count(&self) -> Option<usize> {
        let mut v: Vec<usize> = self.roots.iter().map(Headline::tag_count).collect();
        if v.is_empty() {
            return None;
        }
        v.sort_unstable();
    ...
    /// Histogram of root tag counts to occurrence count.
    #[must_use]
    pub fn root_tag_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for r in &self.roots {
            *m.entry(r.tag_count()).or_insert(0) += 1;
        }
        m
    }
    ...
    /// Total property count across roots only (no recursion).
    #[must_use]
    pub fn total_root_property_count(&self) -> usize {
        self.roots.iter().map(Headline::property_count).sum()
    }

    /// Maximum root property count.
    #[must_use]
    pub fn max_root_property_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::property_count).max()
    }

    /// Minimum root property count.
    #[must_use]
    pub fn min_root_property_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::property_count).min()
    }

    /// Integer mean root property count (`0` when no roots).
    #[must_use]
    ...
    #[must_use]
    pub fn median_root_property_count(&self) -> Option<usize> {
        let mut v: Vec<usize> = self.roots.iter().map(Headline::property_count).collect();
        if v.is_empty() {
            return None;
        }
        v.sort_unstable();
    ...
    /// Histogram of root property counts to occurrence count.
    #[must_use]
    pub fn root_property_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for r in &self.roots {
            *m.entry(r.property_count()).or_insert(0) += 1;
        }
        m
    }
    ...
    /// Returns min descendant count across headlines (`0` when empty).
    #[must_use]
    pub fn min_descendant_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::descendant_count)
            .min()
    ...
    /// Total descendant count across all headlines.
    #[must_use]
    ...
    /// Integer mean descendant count across headlines (`0` when empty).
    #[must_use]
    ...
    /// Median descendant count across headlines (`None` when empty).
    #[must_use]
    ...
    /// Histogram of descendant counts to occurrence count.
    #[must_use]
    ...
    /// Most common descendant count (lowest wins ties).
    #[must_use]
    ...
    /// Median root descendant count (`None` when no roots).
    #[must_use]
    pub fn median_root_descendant_count(&self) -> Option<usize> {
        let mut v: Vec<usize> = self.roots.iter().map(Headline::descendant_count).collect();
        if v.is_empty() {
            return None;
        }
        v.sort_unstable();
    ...
    /// Histogram of root descendant counts to occurrence count.
    #[must_use]
    pub fn root_descendant_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for r in &self.roots {
            *m.entry(r.descendant_count()).or_insert(0) += 1;
        }
        m
    }

    /// Most common root descendant count (lowest wins ties).
    #[must_use]
    pub fn mode_root_descendant_count(&self) -> Option<usize> {
        let mut best: Option<(usize, usize)> = None;
        for (dc, c) in self.root_descendant_count_counts() {
            if best.is_none_or(|(_, bc)| c > bc) {
                best = Some((dc, c));
            }
        }
    ...
    /// Returns min subtree byte count across headlines (`0` when empty).
    #[must_use]
    pub fn min_subtree_byte_count(&self) -> usize {
        self.iter_headlines()
            .into_iter()
            .map(Headline::subtree_byte_count)
            .min()
    ...
    /// Returns total subtree byte count summed across all headlines.
    #[must_use]
    ...
    /// Integer mean subtree byte count across headlines (`0` when empty).
    #[must_use]
    ...
    /// Median subtree byte count across headlines (`None` when empty).
    #[must_use]
    ...
    /// Histogram of per-headline subtree byte counts to occurrence count.
    #[must_use]
    ...
    /// Most common subtree byte count (lowest wins ties).
    #[must_use]
    ...
    /// Returns max level present (`0` if no headlines).
    #[must_use]
    ...
    /// Returns headlines whose title contains `needle` ignoring case.
    #[must_use]
    pub fn headlines_with_title_substring_ignore_case<'a>(
        &'a self,
        needle: &str,
    ) -> Vec<&'a Headline> {
        let lower = needle.to_lowercase();
        self.filter_headlines(|h| h.title().to_lowercase().contains(&lower))
    }
    ...
    /// True iff any headline carries `tag`.
    #[must_use]
    pub fn contains_tag(&self, tag: &str) -> bool {
        self.iter_headlines().into_iter().any(|h| h.has_tag(tag))
    }
    ...
    #[must_use]
    pub fn contains_priority(&self, letter: char) -> bool {
        self.iter_headlines()
            .into_iter()
    ...
    /// Total timestamp count across roots only (no recursion).
    #[must_use]
    pub fn total_root_timestamp_count(&self) -> usize {
        self.roots.iter().map(Headline::timestamp_count).sum()
    }

    /// Maximum root timestamp count.
    #[must_use]
    pub fn max_root_timestamp_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::timestamp_count).max()
    }

    /// Minimum root timestamp count.
    #[must_use]
    pub fn min_root_timestamp_count(&self) -> Option<usize> {
        self.roots.iter().map(Headline::timestamp_count).min()
    }

    /// Integer mean root timestamp count (`0` when no roots).
    #[must_use]
    pub fn mean_root_timestamp_count(&self) -> usize {
        let n = self.roots.len();
        self.total_root_timestamp_count()
            .checked_div(n)
            .unwrap_or(0)
    }

    /// Median root timestamp count (`None` when no roots).
    #[must_use]
    pub fn median_root_timestamp_count(&self) -> Option<usize> {
        let mut v: Vec<usize> = self.roots.iter().map(Headline::timestamp_count).collect();
        if v.is_empty() {
            return None;
        }
        v.sort_unstable();
    ...
    /// Histogram of root timestamp counts to occurrence count.
    #[must_use]
    pub fn root_timestamp_count_counts(&self) -> std::collections::BTreeMap<usize, usize> {
        let mut m = std::collections::BTreeMap::new();
        for r in &self.roots {
            *m.entry(r.timestamp_count()).or_insert(0) += 1;
        }
        m
    }
...
impl Headline {
    ...
    #[must_use]
    pub fn subtree_count_with_tag(&self, tag: &str) -> usize {
        usize::from(self.has_tag(tag))
            + self.descendants().iter().filter(|h| h.has_tag(tag)).count()
    }

    /// Count of headlines in this subtree (self + descendants) with TODO `kw`.
    #[must_use]
    pub fn subtree_count_with_todo(&self, kw: &str) -> usize {
        usize::from(self.todo() == Some(kw))
            + self
                .descendants()
                .iter()
    ...
    /// Count of headlines in this subtree (self + descendants) with priority `letter`.
    #[must_use]
    pub fn subtree_count_with_priority(&self, letter: char) -> usize {
        usize::from(self.priority() == Some(letter))
            + self
                .descendants()
                .iter()
                .filter(|h| h.priority() == Some(letter))
