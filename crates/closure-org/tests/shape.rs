...
#[test]
fn doc_mode_root_tag_count_match() {
    let doc = parse("* A :x:\n* B :y:\n* C :a:b:\n").expect("parse");
    // tag counts 1,1,2 -> mode 1
    assert_eq!(doc.mode_root_tag_count(), Some(1));
}

#[test]
fn doc_mode_root_tag_count_none_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.mode_root_tag_count(), None);
}
...
#[test]
fn doc_mode_root_subtree_size_match() {
    let doc = parse("* A\n* B\n* C\n** c1\n").expect("parse");
    // subtree sizes 1,1,2 -> mode 1
    assert_eq!(doc.mode_root_subtree_size(), Some(1));
}

#[test]
fn doc_mode_root_subtree_size_none_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.mode_root_subtree_size(), None);
}

#[test]
fn doc_min_subtree_word_count_match() {
    let doc = parse("* A\nword one\n** B\nthree more words\n* C\nsolo\n").expect("parse");
    // subtree words: A=2+3=5, B=3, C=1 -> min 1
    assert_eq!(doc.min_subtree_word_count(), 1);
}

#[test]
fn doc_total_subtree_word_count_match() {
    let doc = parse("* A\nword one\n** B\nthree more words\n* C\nsolo\n").expect("parse");
    // 5+3+1 = 9
    assert_eq!(doc.total_subtree_word_count(), 9);
}

#[test]
fn doc_mean_subtree_word_count_match() {
    let doc = parse("* A\nword one\n** B\nthree more words\n* C\nsolo\n").expect("parse");
    // 9 / 3 = 3
    assert_eq!(doc.mean_subtree_word_count(), 3);
}

#[test]
fn doc_subtree_word_count_zero_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.max_subtree_word_count(), 0);
    assert_eq!(doc.min_subtree_word_count(), 0);
    assert_eq!(doc.total_subtree_word_count(), 0);
    assert_eq!(doc.mean_subtree_word_count(), 0);
}
...
#[test]
fn doc_max_min_subtree_link_count_match() {
    let doc = parse("* A\n[[l1]]\n** B\n[[l2]] [[l3]]\n* C\nno links\n").expect("parse");
    // subtree links: A=1+2=3, B=2, C=0 -> max 3, min 0
    assert_eq!(doc.max_subtree_link_count(), 3);
    assert_eq!(doc.min_subtree_link_count(), 0);
}

#[test]
fn doc_total_subtree_link_count_match() {
    let doc = parse("* A\n[[l1]]\n** B\n[[l2]] [[l3]]\n* C\nno links\n").expect("parse");
    // 3+2+0 = 5
    assert_eq!(doc.total_subtree_link_count(), 5);
}

#[test]
fn doc_mean_subtree_link_count_match() {
    let doc = parse("* A\n[[l1]]\n** B\n[[l2]]\n* C\n[[l3]]\n").expect("parse");
    // subtree links: A=1+1=2, B=1, C=1 -> total 4 / 3 = 1
    assert_eq!(doc.mean_subtree_link_count(), 1);
}

#[test]
fn doc_subtree_link_count_zero_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.max_subtree_link_count(), 0);
    assert_eq!(doc.min_subtree_link_count(), 0);
    assert_eq!(doc.total_subtree_link_count(), 0);
    assert_eq!(doc.mean_subtree_link_count(), 0);
}
...
#[test]
fn doc_max_min_subtree_tag_count_match() {
    let doc = parse("* A :x:\n** B :y:z:\n* C\n").expect("parse");
    // subtree distinct tags: A={x,y,z}=3, B={y,z}=2, C={}=0
    assert_eq!(doc.max_subtree_tag_count(), 3);
    assert_eq!(doc.min_subtree_tag_count(), 0);
}

#[test]
fn doc_total_subtree_tag_count_match() {
    let doc = parse("* A :x:\n** B :y:z:\n* C\n").expect("parse");
    // 3+2+0 = 5
    assert_eq!(doc.total_subtree_tag_count(), 5);
}

#[test]
fn doc_mean_subtree_tag_count_match() {
    let doc = parse("* A :x:\n* B :y:\n* C :z:\n").expect("parse");
    // 1+1+1=3, n=3 -> 1
    assert_eq!(doc.mean_subtree_tag_count(), 1);
}

#[test]
fn doc_subtree_tag_count_zero_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.max_subtree_tag_count(), 0);
    assert_eq!(doc.min_subtree_tag_count(), 0);
    assert_eq!(doc.total_subtree_tag_count(), 0);
    assert_eq!(doc.mean_subtree_tag_count(), 0);
}

#[test]
fn doc_median_subtree_tag_count_match() {
    let doc = parse("* A :x:\n** B :y:z:\n* C\n").expect("parse");
    // counts: A=3, B=2, C=0 -> sorted 0,2,3 -> median 2
    assert_eq!(doc.median_subtree_tag_count(), Some(2));
}

#[test]
fn doc_subtree_tag_count_counts_match() {
    let doc = parse("* A :x:\n* B :y:\n* C :a:b:\n").expect("parse");
    // 1,1,2 -> 1->2, 2->1
    let m = doc.subtree_tag_count_counts();
    assert_eq!(m.get(&1), Some(&2));
    assert_eq!(m.get(&2), Some(&1));
}

#[test]
fn doc_mode_subtree_tag_count_match() {
    let doc = parse("* A :x:\n* B :y:\n* C :a:b:\n").expect("parse");
    assert_eq!(doc.mode_subtree_tag_count(), Some(1));
}

#[test]
fn doc_median_subtree_tag_count_none_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.median_subtree_tag_count(), None);
    assert_eq!(doc.mode_subtree_tag_count(), None);
}

#[test]
fn doc_max_min_subtree_todo_count_match() {
    let doc = parse("* TODO A\n** DONE B\n* C\n").expect("parse");
    // subtree distinct TODOs: A={TODO,DONE}=2, B={DONE}=1, C={}=0
    assert_eq!(doc.max_subtree_todo_count(), 2);
    assert_eq!(doc.min_subtree_todo_count(), 0);
}

#[test]
fn doc_total_subtree_todo_count_match() {
    let doc = parse("* TODO A\n** DONE B\n* C\n").expect("parse");
    // 2+1+0 = 3
    assert_eq!(doc.total_subtree_todo_count(), 3);
}

#[test]
fn doc_mean_subtree_todo_count_match() {
    let doc = parse("* TODO A\n* TODO B\n* TODO C\n").expect("parse");
    // 1+1+1=3, n=3 -> 1
    assert_eq!(doc.mean_subtree_todo_count(), 1);
}

#[test]
fn doc_subtree_todo_count_zero_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.max_subtree_todo_count(), 0);
    assert_eq!(doc.min_subtree_todo_count(), 0);
    assert_eq!(doc.total_subtree_todo_count(), 0);
    assert_eq!(doc.mean_subtree_todo_count(), 0);
}
...
#[test]
fn doc_mean_subtree_timestamp_count_match() {
    let doc = parse("* A\n<2026-01-01>\n* B\n<2026-01-02>\n* C\n<2026-01-03>\n").expect("parse");
    // 1+1+1=3, n=3 -> 1
    assert_eq!(doc.mean_subtree_timestamp_count(), 1);
}

#[test]
fn doc_subtree_timestamp_count_zero_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.max_subtree_timestamp_count(), 0);
    assert_eq!(doc.min_subtree_timestamp_count(), 0);
    assert_eq!(doc.total_subtree_timestamp_count(), 0);
    assert_eq!(doc.mean_subtree_timestamp_count(), 0);
}
...
#[test]
fn doc_median_subtree_priority_count_match() {
    let doc = parse("* [#A] X\n** [#B] Y\n*** [#A] Z\n* W\n").expect("parse");
    // X=2, Y=2, Z=1, W=0 -> sorted 0,1,2,2 -> mid midpoint(1,2)=1
    assert_eq!(doc.median_subtree_priority_count(), Some(1));
}

#[test]
fn doc_subtree_priority_count_counts_match() {
    let doc = parse("* [#A] X\n* [#A] Y\n* [#B] Z\n** [#A] W\n").expect("parse");
    // distinct: X=1, Y=1, Z={B,A}=2, W=1
    let m = doc.subtree_priority_count_counts();
    assert_eq!(m.get(&1), Some(&3));
    assert_eq!(m.get(&2), Some(&1));
}

#[test]
fn doc_mode_subtree_priority_count_match() {
    let doc = parse("* [#A] X\n* [#A] Y\n* [#B] Z\n** [#A] W\n").expect("parse");
    assert_eq!(doc.mode_subtree_priority_count(), Some(1));
}

#[test]
fn doc_median_subtree_priority_count_none_when_empty() {
    let doc = parse("").expect("parse");
    assert_eq!(doc.median_subtree_priority_count(), None);
    assert_eq!(doc.mode_subtree_priority_count(), None);
}
...
#[test]
fn doc_priority_diversity_pct_match() {
    let doc = parse("* [#A] X\n* [#A] Y\n* [#B] Z\n* [#C] W\n").expect("parse");
    // 4 priority occurrences, distinct {A,B,C}=3 -> 75
    assert_eq!(doc.priority_diversity_pct(), 75);
}

#[test]
fn doc_priority_diversity_pct_zero_when_none() {
    let doc = parse("* A\n* B\n").expect("parse");
    assert_eq!(doc.priority_diversity_pct(), 0);
}

#[test]
fn doc_title_diversity_pct_match() {
    let doc = parse("* A\n* A\n* B\n* C\n").expect("parse");
    // 4 headlines, distinct {A,B,C}=3 -> 75
    assert_eq!(doc.title_diversity_pct(), 75);
}

#[test]
fn doc_title_diversity_pct_zero_when_empty() {
    let doc = parse("preamble only\n").expect("parse");
...
#[test]
fn doc_property_key_len_none_when_no_props() {
    let doc = parse("* A\n").expect("parse");
    assert_eq!(doc.max_property_key_len(), None);
    assert_eq!(doc.min_property_key_len(), None);
...
#[test]
fn doc_property_value_len_none_when_no_props() {
    let doc = parse("* A\n").expect("parse");
    assert_eq!(doc.max_property_value_len(), None);
    assert_eq!(doc.min_property_value_len(), None);
...
#[test]
fn doc_has_any_archived_alias_match() {
    let doc = parse("* A :ARCHIVE:\n").expect("parse");
    assert!(doc.has_any_archived());
}

#[test]
fn doc_has_any_scheduled_alias_match() {
    let doc = parse("* A\nSCHEDULED: <2026-05-30 Sat>\n").expect("parse");
    assert!(doc.has_any_scheduled());
}

#[test]
fn doc_has_any_deadline_alias_match() {
    let doc = parse("* A\nDEADLINE: <2026-05-30 Sat>\n").expect("parse");
    assert!(doc.has_any_deadline());
}

#[test]
fn doc_has_any_closed_alias_match() {
    let doc = parse("* A\nCLOSED: [2026-05-30 Sat]\n").expect("parse");
    assert!(doc.has_any_closed());
}

#[test]
fn doc_has_any_comment_alias_match() {
    let doc = parse("* COMMENT A\n").expect("parse");
    assert!(doc.has_any_comment());
}

#[test]
fn doc_has_any_planning_alias_match() {
    let doc = parse("* A\nSCHEDULED: <2026-05-30 Sat>\n").expect("parse");
    assert!(doc.has_any_planning());
}

#[test]
fn doc_has_any_footnote_alias_match() {
    let doc = parse("* A\nbody[fn:1]\n").expect("parse");
    assert!(doc.has_any_footnote());
}

#[test]
fn doc_has_any_macro_alias_match() {
    let doc = parse("* A\n{{{m(x)}}}\n").expect("parse");
    assert!(doc.has_any_macro());
}

#[test]
fn doc_has_any_cookie_alias_match() {
    let doc = parse("* TODO A [1/2]\n** TODO X\n** DONE Y\n").expect("parse");
    let _ = doc.has_any_cookie();
}
